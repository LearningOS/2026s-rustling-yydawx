...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller has guaranteed that `address` contains a mutable
    // reference to a valid `u32` value, so we can safely convert it back to
    // a pointer and write to it.
    unsafe {
